    ...
    <div style={{ fontFamily: 'Segoe UI, system-ui, sans-serif', background: '#fff', color: '#0F172A', minHeight: '100vh' }}>
      ...
      <section style={{ background: '#0F172A', padding: '50px 20px', textAlign: 'center', color: '#fff' }}>
        ...
        <p style={{ fontSize: '14px', color: '#94A3B8', maxWidth: '420px', margin: '0 auto 24px' }}>
          Check how visible any business is on Google — completely free. No signup needed.
        </p>

        <div style={{ maxWidth: '440px', margin: '0 auto' }}>
          <input
            placeholder="Enter business name e.g. Lax Furniture"
            value={businessName}
            onChange={e => setBusinessName(e.target.value)}
            onKeyDown={e => e.key === 'Enter' && checkVisibility()}
            style={{ width: '100%', padding: '14px 16px', borderRadius: '8px', border: 'none', fontSize: '15px', marginBottom: '10px', outline: 'none', color: '#0F172A', boxSizing: 'border-box' }}
          />
          <input
            ...
            onKeyDown={e => e.key === 'Enter' && checkVisibility()}
            style={{ width: '100%', padding: '14px 16px', borderRadius: '8px', border: 'none', fontSize: '15px', marginBottom: '12px', outline: 'none', color: '#0F172A', boxSizing: 'border-box' }}
          />
          <button
          ...
          >
            {loading ? 'Checking Google...' : 'Check My Visibility Score'}
          </button>
        </div>
      </section>
      ...
      {result && (
        <section style={{ maxWidth: '520px', margin: '0 auto', padding: '32px 20px' }}>
          ...
          {/* GOOGLE VISIBILITY RESULT */}
          <div style={{ background: '#F8FAFC', border: '1px solid #E2E8F0', borderRadius: '12px', padding: '20px', marginBottom: '16px' }}>
            <div style={{ fontSize: '12px', fontWeight: 700, color: '#475569', textTransform: 'uppercase', marginBottom: '12px' }}>Google Visibility</div>
            ...
            {result.googleData.found ? (
              <div>
                ...
                <div style={{ fontWeight: 700, fontSize: '14px', marginBottom: '10px', color: '#0F172A' }}>{result.googleData.business.name}</div>

                {result.googleData.business.address && (
                  <div style={{ fontSize: '12px', color: '#475569', marginBottom: '6px' }}>📍 {result.googleData.business.address}</div>
                )}
                {result.googleData.business.phone && (
                  <div style={{ fontSize: '12px', color: '#475569', marginBottom: '6px' }}>📞 {result.googleData.business.phone}</div>
                )}
                {result.googleData.business.rating && (
                  <div style={{ fontSize: '12px', color: '#475569', marginBottom: '12px' }}>⭐ {result.googleData.business.rating} ({result.googleData.business.reviewCount} reviews)</div>
                )}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px' }}>
                  {[
                    ...
                    { label: 'Website', done: result.googleData.breakdown.website },
                    ...
                    { label: 'Verified Status', done: result.googleData.breakdown.verified },
                  ].map(item => (
                    <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '5px 0', fontSize: '12px' }}>
                ...
                {!result.googleData.breakdown.website && (
                  <div style={{ background: '#FFF7ED', border: '1px solid #FED7AA', borderRadius: '8px', padding: '12px', marginTop: '14px' }}>
                    <div style={{ fontSize: '12px', fontWeight: 700, color: '#9A3412', marginBottom: '4px' }}>Missing: Website</div>
                    <div style={{ fontSize: '12px', color: '#475569' }}>Add a free Cloutinet page as your website to boost your Google score.</div>
                  </div>
                )}
              </div>
            ) : (
              <div style={{ textAlign: 'center', padding: '20px 0' }}>
                ...
                <div style={{ fontSize: '12px', color: '#64748B' }}>This business has no visible Google presence. Customers cannot find it online.</div>
              </div>
            )}
          </div>
          ...
          <div style={{ background: '#F8FAFC', border: '1px solid #E2E8F0', borderRadius: '12px', padding: '20px', marginBottom: '20px' }}>
            <div style={{ fontSize: '12px', fontWeight: 700, color: '#475569', textTransform: 'uppercase', marginBottom: '12px' }}>Cloutinet Page Status</div>
            ...
            {result.cloutProfile ? (
              <div>
                ...
                <div style={{ fontSize: '12px', color: '#475569', marginBottom: '10px' }}>Cloutinet Score: {result.cloutScore}/100 · {result.productCount} products listed</div>
                <a href={'/store/' + result.cloutProfile.business_slug} style={{ display: 'inline-block', background: '#0F172A', color: '#fff', padding: '8px 18px', borderRadius: '6px', textDecoration: 'none', fontSize: '12px', fontWeight: 700 }}>View Store Page →</a>
              </div>
            ) : (
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
                ...
                <div style={{ fontSize: '12px', color: '#64748B', marginBottom: '12px' }}>Create a free Cloutinet page to get a Google-indexed business page, product listings, and direct WhatsApp leads.</div>
          ...
          {result.googleData.found && result.googleData.googleScore < 100 && (
            <div style={{ background: '#F0FDF4', border: '1px solid #BBF7D0', borderRadius: '12px', padding: '16px', marginBottom: '20px' }}>
              ...
              {!result.googleData.breakdown.website && <div style={{ fontSize: '12px', color: '#166534', marginBottom: '6px' }}>• Add a website — use your free Cloutinet page</div>}
              {!result.googleData.breakdown.hours && <div style={{ fontSize: '12px', color: '#166534', marginBottom: '6px' }}>• Add business hours to your Google Business Profile</div>}
              {!result.googleData.breakdown.photos && <div style={{ fontSize: '12px', color: '#166634', marginBottom: '6px' }}>• Add photos to your Google Business Profile</div>}
              {!result.googleData.breakdown.reviews && <div style={{ fontSize: '12px', color: '#166534', marginBottom: '6px' }}>• Ask customers to leave Google reviews</div>}
      ...
      {!result && !loading && (
        <section style={{ maxWidth: '520px', margin: '0 auto', padding: '32px 20px', textAlign: 'center' }}>
          <h2 style={{ fontSize: '18px', fontWeight: 700, marginBottom: '8px', color: '#0F172A' }}>What does this tool check?</h2>
          <p style={{ fontSize: '13px', color: '#64748B', marginBottom: '24px' }}>We search Google for your business and score its online presence based on 9 key factors that affect how easily customers can find you.</p>
