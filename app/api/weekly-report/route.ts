...
export async function GET(req: NextRequest) {
  ...
  for (const profile of profiles || []) {
    ...
    if (pendingTotal > 0) {
      insights.push(`${pending?.[0]?.currency || 'NGN'} ${pendingTotal.toLocaleString()} in pending payments.`)
    }
    ...
    const html = `
      <h2>Your weekly Cloutinet digest</h2>
      <p>Here's what's happening with ${profile.business_name}:</p>
      <ul>${insights.map(i => `<li>${i}</li>`).join('')}</ul>
      <p><a href="https://cloutinet.online/dashboard">View your dashboard</a></p>
    `

    await sendEmail(profile.email, `Your weekly update — ${profile.business_name}`, html)
    sent++
  }
