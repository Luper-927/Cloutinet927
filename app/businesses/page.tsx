...
export async function generateMetadata() {
  return {
    ...
    description: 'Browse verified businesses across Nigeria. Find restaurants, salons, shops, fashion, electronics and more. Contact businesses directly on WhatsApp.',
  }
}
  ...
  return (
    <div style={{ fontFamily: 'Segoe UI, system-ui, sans-serif', background: '#fff', color: '#0F172A' }}>
      ...
      <section style={{ background: '#0F172A', padding: '40px 20px', textAlign: 'center', color: '#fff' }}>
        <h1 style={{ fontSize: '24px', fontWeight: 800, marginBottom: '8px' }}>Find Businesses in Nigeria</h1>
        <p style={{ fontSize: '14px', color: '#94A3B8', maxWidth: '400px', margin: '0 auto' }}>Browse verified businesses across Nigeria. Contact them directly on WhatsApp.</p>
        <div style={{ marginTop: '12px', fontSize: '13px', color: '#64748B' }}>{businesses.length} businesses listed</div>
      </section>

      <section style={{ maxWidth: '700px', margin: '0 auto', padding: '24px 16px' }}>

        <h2 style={{ fontSize: '16px', fontWeight: 700, marginBottom: '16px', color: '#0F172A' }}>Browse by City</h2>
        ...
        <h2 style={{ fontSize: '16px', fontWeight: 700, marginBottom: '16px', color: '#0F172A' }}>Browse by Category</h2>
        ...
        <h2 style={{ fontSize: '16px', fontWeight: 700, marginBottom: '16px', color: '#0F172A' }}>Recently Added Businesses</h2>
        {businesses.length === 0 ? (
          <p style={{ color: '#94A3B8', fontSize: '13px', textAlign: 'center', padding: '20px' }}>No businesses listed yet.</p>
        ) : (
              ...
              return (
                <Link key={b.business_slug} href={'/store/' + b.business_slug} style={{
                  ...
                  <div>
                    ...
                    {b.business_category ? <div style={{ fontSize: '11px', color: '#475569', marginBottom: '2px' }}>{b.business_category}</div> : null}
                    {b.location ? <div style={{ fontSize: '11px', color: '#94A3B8' }}>{b.location}</div> : null}
                  </div>
                  <div style={{ color: '#0F172A', fontSize: '18px' }}>→</div>
                </Link>
              )
            })}
          </div>
      ...
      <footer style={{ background: '#F8FAFC', padding: '24px', textAlign: 'center', borderTop: '1px solid #E2E8F0', marginTop: '20px' }}>
        <div style={{ fontSize: '13px', color: '#0F172A', fontWeight: 700, marginBottom: '8px' }}>Is your business listed here?</div>
        <p style={{ fontSize: '12px', color: '#64748B', marginBottom: '12px' }}>Join businesses getting found on Google with Cloutinet.</p>
        <Link href="/auth" style={{ background: '#0F172A', color: '#fff', padding: '10px 24px', borderRadius: '6px', textDecoration: 'none', fontSize: '13px', fontWeight: 700 }}>List Your Business Free →</Link>
      </footer>
    </div>
  )
}
